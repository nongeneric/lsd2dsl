...
void AbbreviationDictionaryDecoder::Read(IBitStream *bstr){
    int len = bstr->read(32);
...
void AbbreviationDictionaryDecoder::DecodeHeading(IBitStream *bstr, unsigned len, std::u16string &res) {
    res.clear();
    unsigned symIdx;
...
bool AbbreviationDictionaryDecoder::DecodeArticle(IBitStream *bstr, std::u16string &res) {
    return UserDictionaryDecoder::DecodeArticle(bstr, res, _prefix, _ltArticles, _articleSymbols);
}

bool AbbreviationDictionaryDecoder::DecodePrefixLen(IBitStream &bstr, unsigned &len) {
    return _ltPrefixLengths.Decode(bstr, len);
}

bool AbbreviationDictionaryDecoder::DecodePostfixLen(IBitStream &bstr, unsigned &len) {
    return _ltPostfixLengths.Decode(bstr, len);
}

bool AbbreviationDictionaryDecoder::ReadReference1(IBitStream &bstr, unsigned &reference) {
    return readReference(bstr, reference, _huffman1Number);
}

bool AbbreviationDictionaryDecoder::ReadReference2(IBitStream &bstr, unsigned &reference) {
