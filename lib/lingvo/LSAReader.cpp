...
std::u16string readLSAString(common::IRandomAccessStream* bstr) {
    std::u16string res;
...
LSAReader::LSAReader(common::IRandomAccessStream *bstr)
    : _bstr(bstr)
{
    assert(bstr);
...
void LSAReader::dump(std::filesystem::path path, Log& log) {
    ...
    for (size_t i = 0; i < _entries.size(); ++i) {
        ...
        common::createWav(samples, wav, info.rate, info.channels);
